"""Combine beacons together as the real shape, and returns all the combinations."""
...
class BeaconSorter:
    ...
    def _sort_comparison(self, beacons, new_objects_detected):
        """Classify objects depending on their previous position."""
    ...
    def _find_beacons_prev(self, beacons, new_objects_detected):
        """Combine potential beacons using previous ones."""
        ...
        for i, bA in enumerate(sorting_list["A"]):
            for j, bB in enumerate(sorting_list["B"]):
                ...
                for k, bC in enumerate(sorting_list["C"]):
                    ...
                    for m, bD in enumerate(sorting_list["D"]):
                        for key in list(flags.keys())[3:]:
                            flags[key] = False
                        if approx(
                            dt(bA, bD), self._dst_beacons[("A", "D")], self._dst_tol
                        ):
                            flags[("A", "D")] = True
                            indexes2.append([i, -1, -1, m])
                        if approx(
                            dt(bB, bD), self._dst_beacons[("B", "D")], self._dst_tol
                        ):
                            flags[("B", "D")] = True
                            indexes2.append([-1, j, -1, m])
                        if approx(
                            dt(bC, bD), self._dst_beacons[("C", "D")], self._dst_tol
                        ):
                            flags[("C", "D")] = True
                            indexes2.append([-1, -1, k, m])
                        if (
                            flags[("A", "B")]
                            and flags[("A", "D")]
                            and flags[("B", "D")]
                        ):
                            indexes3.append([i, j, -1, m])
                        if (
                            flags[("A", "C")]
                            and flags[("A", "D")]
                            and flags[("C", "D")]
                        ):
                            indexes3.append([i, -1, k, m])
                        if (
                            flags[("B", "C")]
                            and flags[("B", "D")]
                            and flags[("C", "D")]
                        ):
                            indexes3.append([-1, j, k, m])
                        if (
                            flags[("A", "B")]
                            and flags[("A", "C")]
                            and flags[("B", "C")]
                            ...
                            indexes4.append([i, j, k, m])

        if indexes4 != []:
            unique_indexes4 = np.unique(np.array(indexes4), axis=0)
            clean_beacons4 = []
            for index in unique_indexes4:
                clean_beacons4.append(
                    [sorting_list[chr(i + 65)][index[i]] for i in range(4)]
                )

            beacons_list = clean_beacons4.copy()
            for beacons in beacons_list:
                sign_res = [
                    get_angle_sign([beacons[i] for i in range(4) if i != 3 - j])
                    for j in range(4)
                ]
        ...
        if indexes3 != []:
            ...
            for index in unique_indexes3:
                clean_beacons3.append(
                    [
                        sorting_list[chr(i + 65)][index[i]] if index[i] >= 0 else None
                        for i in range(4)
                    ]
                )
            ...
            clean_beacons3 = [
                beacons
                for beacons in clean_beacons3
                if get_angle_sign([beacon for beacon in beacons if beacon is not None])
                == self._sign_vect_product[
                    [3 - i for i in range(len(beacons)) if beacons[i] is None][0]
                ]
            ]
            if clean_beacons3 != []:
    ...
    def _find_beacons_naive(self, new_objects_detected):
        """Combine potential beacons without any previous knowledges."""
        ...
        while len(olist) >= 2:
            ...
            beacon1 = olist.pop()
            dict_indexes = {
                (letters[i], letters[j]): [] for i in range(4) for j in range(i + 1, 4)
            }
                ...
                for i in range(3):
                    for ind1 in dict_indexes[sorttuple(clet, olet[i])]:
                        for ind2 in dict_indexes[sorttuple(clet, olet[(i + 1) % 3])]:
                            ...
                            if approx(
                                dt(olist[ind1], olist[ind2]),
                                self._dst_beacons[
                                    sorttuple(olet[i], olet[(i + 1) % 3])
                                ],
                                self._dst_tol,
                            ):
                                temp_list = [None] * 4
                                ...
                                temp_list[toint(olet[(i + 1) % 3])] = olist[ind2]
                                none_index = next(
                                    (i for i, x in enumerate(temp_list) if x is None),
                                    None,
                                )
                                if (
                                    get_angle_sign(
                                        [b for b in temp_list if b is not None]
                                    )
                                    == self._sign_vect_product[3 - none_index]
                                ):
                                ...
                                if i == 0:
                                    ...
                                    for ind3 in dict_indexes[
                                        sorttuple(clet, olet[(i + 2) % 3])
                                    ]:
                                        if approx(
                                            dt(olist[ind1], olist[ind3]),
                                            self._dst_beacons[
                                                sorttuple(olet[i], olet[(i + 2) % 3])
                                            ],
                                            self._dst_tol,
                                        ) and approx(
                                            dt(olist[ind2], olist[ind3]),
                                            self._dst_beacons[
                                                sorttuple(
                                                    olet[(i + 1) % 3], olet[(i + 2) % 3]
                                                )
                                            ],
                                            self._dst_tol,
                                        ):
                                            temp_list[toint(olet[(i + 2) % 3])] = olist[
                                                ind3
                                            ]
                                            sign_res = [
                                                get_angle_sign(
    ...
    def _find_possible_beacons(self, previous_beacons, new_objects_detected):
