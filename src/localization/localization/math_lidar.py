...
def chgt_base_plateau_to_robot(pt, robot_frame):
...
def chgt_base_robot_to_plateau(pt, robot_frame):
    cos_angle, sin_angle = np.cos(robot_frame[2]), np.sin(robot_frame[2])
    OtoR = np.array(
        [
            [cos_angle, -sin_angle, robot_frame[0]],
...
def _calcul_erreur(beacons, position, fixed_beacons):
    sum = 0
    for b, fb in zip(beacons, fixed_beacons):
        if b is not None:
            sum += dt2(chgt_base_robot_to_plateau(b, position), fb)
    return sum
...
def find_position(beacons_list, fixed_beacons, nombre, boundaries):
    ...
    for beacons in beacons_list:
        ...
        dt_min = _calcul_erreur(beacons, position_found, fixed_beacons)
        err.append(dt_min)
