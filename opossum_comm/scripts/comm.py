...
class Communication(Node):
    ...
    def _init_parameters(self: Node) -> None:
        ...
        with open(self.csv_file, "w", newline="") as csvfile:
            self.get_logger().info(f"Creating the file")
            writer = csv.writer(csvfile)
            writer.writerow(["x", "y", "theta", "vx", "vy", "vtheta", "opt0", "opt1", "opt2"])
    ...
    def check_and_publish_asserv(self, data):
        ...
        if splitted_data[0] == "DEBUG" and len(splitted_data) == 10:
            ...
            self.buffer.append((pos.x, pos.y, pos.z, vel.x, vel.y, vel.z, opt0, opt1, opt2))
            if len(self.buffer) >= 1000:
                write_to_csv(self.csv_file, self.buffer)
                self.get_logger().info(f"Dumped {len(self.buffer)} records to {self.csv_file}.")
...
def write_to_csv(filename, data):
    ...
    Args:
        ...
        data (list): A list of tuples where each tuple contains 6 data points.
    """
...
def main(args=None):
    ...
    try:
        ...
        zynq_comm_simu_node.get_logger().info(f"Data collection terminated, buffer_len: {zynq_comm_simu_node.buffer} .")
        if zynq_comm_simu_node.buffer or True:
            write_to_csv(zynq_comm_simu_node.csv_file, zynq_comm_simu_node.buffer)
            zynq_comm_simu_node.get_logger().info(f"Dumped remaining {len(zynq_comm_simu_node.buffer)} records to {zynq_comm_simu_node.csv_file} before exit.")
