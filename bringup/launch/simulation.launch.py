...
def generate_launch_description():
    ...
    launch_pigpiod_em = ExecuteProcess(
        cmd=[
            "python3",
            "{}Pigpiod_Emulator/tools/pigpiod_pepino_interface.py".format(UTILS_PATH),
        ],
    )

    launch_rqt = Node(package="rqt_gui", executable="rqt_gui", name="rqt_gui")
    node_lidar_em = Node(
    ...
    ld.add_action(TimerAction(period=2.0, actions=[launch_pigpiod_em]))
    ld.add_action(
        TimerAction(period=4.0, actions=[node_receiver, node_sender, node_lidar_em])
    )
