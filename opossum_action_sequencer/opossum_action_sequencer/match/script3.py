...
class Script:
    ...
    def run(self, node):
        ...
        node.end_zone = Position(0.35, 1.7, 1.6)
        node.middle = Position(0.7, 1.4, 1.6)
        ...
        node.wait_for_motion()
        node.send_raw("VMAX 1.5")
        node.move_to(Position(0.7, 0.4, 1.6))
        node.wait_for_motion()
        node.send_raw("VMAX 1.5")
        node.move_to(Position(0.7, 1.4, 1.6))
        node.wait_for_motion()
        node.send_raw("VMAX 1.5")
        node.move_to(Position(0.7, 0.4, 1.6))
        node.wait_for_motion()
        ...
        node.move_to(Position(0.35, 1.7, 1.6))
        node.wait_for_motion()
